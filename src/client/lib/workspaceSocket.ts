 ...
 * `resolveRole` boundary), or (production) the caller isn't authenticated at
 * all. It is distinct from "closed", which still retries.
 */
 ...
 *
 * A browser WebSocket can't tell you WHY the upgrade failed — a 401/403 from
 * the Worker's authorization boundary and a dropped network connection both
 * just fire a generic `close`/`error` event. So before opening the socket we
 * do a cheap HTTP preflight against the same authorization boundary
 * (`/snapshot`): an explicit 401/403 means "not authorized, never will be
 * until membership/session changes" — stop retrying and surface a clear
 * denied state. Any other outcome (network error, 5xx, timeout) falls
 * through to the normal WebSocket attempt with its existing backoff/retry
 * behavior, so a genuinely flaky network doesn't get misclassified as denied.
 *
 * In production this never sends userId/displayName/role as query params —
 * identity is carried entirely by the Access session at the edge, same as
 * every other request to this origin.
 */
export class WorkspaceSocket {
  ...
  private async preflightThenConnect(): Promise<void> {
    ...
    const qs = identityQuery(this.opts.mode);

    try {
      const res = await fetch(withQuery(`${workspaceBase(this.opts.workspaceId, this.opts.mode)}/snapshot`, qs));
  ...
  private openSocket(): void {
    ...
    const qs = identityQuery(this.opts.mode);
    const url = `${scheme}://${location.host}${withQuery(
      `${workspaceBase(this.opts.workspaceId, this.opts.mode)}/ws`,
      qs,
    )}`;
